...
# -*- coding: utf-8 -*-
"""
🤖 SHEIN Price Calculator Bot v2.5 - Exact Float Precision (Fix: Keep Price History)
"""
...
async def category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # تم التعديل هنا: إرسال رسالة جديدة بدل التعديل لضمان بقاء الهستوري سليم
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="💰 الآن، أرسل <b>سعر المنتج بالريال السعودي</b> كما يظهر بالتطبيق:",
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == 'start_again':
        # إزالة الزر القديم من رسالة السعر لحماية واجهة المحادثة، دون مسح النص نفسه
        try:
        ...
        # التعديل الجوهري: إرسال رسالة جديدة تماماً لكي يظهر السؤال بالأسفل وتبقى الفاتورة ظاهرة
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="🛍️ <b>اختر فئة المنتج الجديد:</b>",
